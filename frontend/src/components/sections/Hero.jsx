...
const Hero = ({ profile }) => {
  ...
  const roles = profile?.roleTitles?.length > 0 ? profile.roleTitles : [];
          ...
          >
            ...
            <h1 className="text-5xl md:text-7xl font-bold tracking-tight text-white leading-tight">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent">
                {profile?.name || 'Rudranil Koley'}
              </span>
            </h1>

            <div className="h-12 md:h-16 flex items-center">
              <p className="text-2xl md:text-3xl font-medium text-gray-300">
                I am a <span className="text-white font-bold">{roles[roleIndex] || 'Data Professional'}</span>
                {roles.length > 1 && <span className="animate-pulse">|</span>}
              </p>
            </div>

            <p className="text-lg text-gray-400 max-w-xl leading-relaxed">
              {profile?.bio || 'Passionate about transforming raw data into meaningful business insights.'}
            </p>
            ...
            <div className="flex flex-wrap gap-4 pt-4">
              ...
              <a href="mailto:rudranilkoley64@gmail.com" className="border border-white/20 hover:border-white/40 text-white px-8 py-3 rounded-xl font-medium flex items-center gap-2 transition-all hover:scale-105 active:scale-95">
